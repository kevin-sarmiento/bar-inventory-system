...
BEGIN
    IF TG_OP = 'INSERT' THEN
        v_data := to_jsonb(NEW);
        v_pk := COALESCE(
                v_data->>'user_id', v_data->>'role_id', v_data->>'permission_id',
                v_data->>'category_id', v_data->>'supplier_id', v_data->>'unit_id',
                v_data->>'location_id', v_data->>'product_id', v_data->>'recipe_id',
                v_data->>'menu_item_id', v_data->>'sale_id', v_data->>'transaction_id',
                v_data->>'physical_count_id', v_data->>'audit_id',
                v_data->>'stock_balance_id', v_data->>'transaction_item_id', v_data->>'sale_item_id',
                v_data->>'physical_count_item_id', v_data->>'recipe_item_id', '0'
                );
